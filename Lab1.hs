...
{- Alternative solution (for both compare functions,
for it to work with the test function):
comparePower2 :: Integer -> Integer -> Property
comparePower2 n k = (k >= 0) ==> power n k == power2 n k -}

-- Test functions
generateT :: [Bool]
generateT = [comparePower1 n k && comparePower2 n k | (n,k) <- testcases]
  where testcases = [(3,0),(6,5),(4,(-1))]

test :: Bool
test = generateT == [True, True, True]
